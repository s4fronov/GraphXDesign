        ...
        public void DrawLine(int x1, int y1, int x2, int y2, bool drawFirstDot = false)
        {
            ...
            for (int i = 1; i < dotList.Count; i++)
            {
                if (dotList[i].Item1 < dotList[i - 1].Item1)
                    DrawDotBorder(dotList[i].Item1, dotList[i].Item2, Direction.Left);
                if (dotList[i].Item1 > dotList[i - 1].Item1)
                    DrawDotBorder(dotList[i].Item1, dotList[i].Item2, Direction.Right);
                if (dotList[i].Item2 < dotList[i - 1].Item2)
                    DrawDotBorder(dotList[i].Item1, dotList[i].Item2, Direction.Up);
                if (dotList[i].Item2 > dotList[i - 1].Item2)
                    DrawDotBorder(dotList[i].Item1, dotList[i].Item2, Direction.Down);
            }
        }

        enum Direction { Up, Down, Left, Right }
        private void DrawDotBorder( int x, int y, Direction direction)
        {
            int x1 = x - BrushSize / 2;
            int x2 = x1 + BrushSize - 1;
            int y1 = y - BrushSize / 2;
            ...
            if (direction == Direction.Up)
                for (int i = x1; i <= x2; i++)
                {
                    Canvas.GetCanvas.SetPixel(i, y1, BrushColor);
                }

            if (direction == Direction.Down)
                for (int i = x1; i <= x2; i++)
                {
                    Canvas.GetCanvas.SetPixel(i, y2, BrushColor);
                }

            if (direction == Direction.Left)
                for (int j = y1; j <= y2; j++)
                {
                    Canvas.GetCanvas.SetPixel(x1, j, BrushColor);
                }

            if (direction == Direction.Right)
                for (int j = y1; j <= y2; j++)
                {
                    Canvas.GetCanvas.SetPixel(x2, j, BrushColor);
                }
        }
    }
}
